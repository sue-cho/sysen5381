...
result1 = agent_run(
    ...
    model=MODEL,
    output="tools",
    tools=[tool_get_shortages],
    namespace=globals(),
)

# result1 is the tool_calls list; the DataFrame is in the first tool's "output"
if isinstance(result1, list) and result1:
    result1 = result1[0].get("output", result1)

# result1 should now be the DataFrame from get_shortages
