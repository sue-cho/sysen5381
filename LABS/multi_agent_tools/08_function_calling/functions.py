...
def _parse_tool_arguments(raw):
    """Ollama may return `arguments` as a JSON string or a dict (API version dependent)."""
    if isinstance(raw, dict):
        return raw
    if isinstance(raw, str):
        return json.loads(raw)
    raise TypeError(f"tool arguments must be str or dict, got {type(raw)}")


# 1. AGENT FUNCTION ###################################

def agent(messages, model=DEFAULT_MODEL, output="text", tools=None, all=False, namespace=None):
    """
    ...
    namespace : dict, optional
        Globals dict where tool implementations live (e.g. pass globals() from
        your script). If None, only functions defined in this module can run as tools.
...
def agent_run(role, task, tools=None, output="text", model=DEFAULT_MODEL, namespace=None):
    """
    ...
    namespace : dict, optional
        Passed to agent() for tool lookup (use globals() from your script).
    ...
    # Run the agent
    resp = agent(messages=messages, model=model, output=output, tools=tools, namespace=namespace)
    return resp
