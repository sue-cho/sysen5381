...
# Install python libraries (see also requirements_embed.txt in this folder):
#   pip install -r requirements_embed.txt
# Or: pip install sentence-transformers sqlite-vec requests python-dotenv
# sentence-transformers pulls PyTorch; versions must match:
# - PyTorch >= 2.4 is required by recent sentence-transformers/transformers.
# - NumPy 2.x often breaks older PyTorch (use numpy<2 OR upgrade torch to >=2.4).
# If you see "PyTorch >= 2.4 is required" or NumPy/_ARRAY_API errors, run:
#   pip install "numpy>=1.26,<2" "torch>=2.4" --upgrade
