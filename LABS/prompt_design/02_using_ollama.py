...
# Verify Ollama server is available before sending chat request
try:
    requests.get(f"{OLLAMA_HOST}/api/tags", timeout=2)
except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
    print("❌ Cannot connect to Ollama at localhost:11434.")
    print("   Start the Ollama server first, then run this script again.")
    print("   Options:")
    print("   - Run in another terminal: ollama serve")
    print("   - Or use the course script: ./06_agents/01_ollama.sh")
    print("   Then pull a model if needed: ollama pull smollm2:1.7b")
    raise SystemExit(1) from e

# Note: Ensure your model is pulled (e.g. ollama pull smollm2:1.7b or ollama list)
...
messages = [
    {
        "role": "system",
        "content": "You are a data analyst. Your name is Jerry. You can only talk about mice and cheese. "
    },
    {
        "role": "user",
        "content": "Hello, what is the cheese market doing?"
    }
]
